...
- [ui.gluestack.io](https://ui.gluestack.io/) - A set of ready-to-use universal components that can be added to any project and styled to fit your needs. You can even customize the components as desired.
...
- **Animation**: gluestack-ui provides a plugin to add support for Animation using Animation libraries like [@legendapp/motion](https://legendapp.com/open-source/motion/), [framer-motion](https://www.framer.com/motion/), and [moti](https://moti.fyi/). [Check here](https://style.gluestack.io/docs/plugins/animation-plugin).
...
gluestack-ui builds upon the React Native Web components by adding accessibility props, focus management, and other functionality to ensure that the components are accessible and performant. But, gluestack-style doesn’t use React Native Web’s styling engine (which is highly performant)! gluestack-ui uses babel-plugin (compiler), which precompiles the declarative styling object into CSS classes and declarations and injects them directly during compile-time leading to zero style calculation during the runtime.

## Why use gluestack-ui, when we already have NativeBase?gluestack-ui is a React library that provides universal, unstyled, and accessible components. It includes keyboard accessibility and focus management features, making it a great choice for developers who prioritize accessibility and usability.
...
We have also shifted from prop based APIs in NativeBase to compound APIs because it provides more cohesive, robust and consistent interface for developers.
