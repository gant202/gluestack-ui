...
gluestack-ui prioritizes performance and is optimized for even complex applications with many elements. **With a babel plugin for pre-computing styles during the build process, and runtime performance optimizations, gluestack-ui delivers high performance styling**.

We tested the performance of our landing page, built with `gluestack-ui` and a large number of elements, using Lighthouse and achieved a **high score of 100 for Performance, 97 for Accessibility, 92 for Best Practices, and 100 for SEO**. You can check out the website and see the Lighthouse report for yourself.
